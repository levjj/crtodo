...
function trash_ui(title) {
  return $('<div class="todogroup" />')
    .append($('<img src="/images/Trash.png" alt="Trash"/>'))
    .droppable({
...
function finish_ui(title) {
  return $('<div class="todogroup" />')
    .append($('<img src="/images/Ok.png" alt="Finish"/>'))
    .droppable({
      hoverClass: 'ui-widget-content',
      drop: function(event, ui) {
        ui.draggable.detach();
        var index = ui.draggable.attr('title');
        $.post('api/' + title + '/' + index, {_method: 'put'});
  	  }
    });
}
